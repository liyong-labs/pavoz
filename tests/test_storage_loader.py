...
from stageflow import StageflowStorageError, load_storage
from stageflow.storage import FileStorage


def test_loads_builtin_file_storage(tmp_path):
    """stageflow 自带的 FileStorage 直接可加载."""
    storage = load_storage("stageflow.storage.FileStorage", root_dir=str(tmp_path))
...
def test_loads_with_kwargs():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        storage = load_storage("stageflow.storage.FileStorage", root_dir=tmp)
        storage.put("k", {"v": 1})
        assert storage.get("k") == {"v": 1}


def test_format_error_missing_colon():
    with pytest.raises(StageflowStorageError, match="格式错误"):
        load_storage("nocolon")


def test_missing_module_error_mentions_module_path():
    with pytest.raises(StageflowStorageError, match="不可"):
        load_storage("nonexistent.package.path:SomeClass")


def test_wrong_class_name_error_lists_module_exports():
    """AttributeError → 提示可用类."""
    with pytest.raises(StageflowStorageError, match="不在模块"):
        load_storage("stageflow.storage:WrongClassName")


def test_wrong_kwargs_error():
    """TypeError → 提示 kwargs 不匹配."""
    with pytest.raises(StageflowStorageError, match="kwargs"):
        load_storage("stageflow.storage.FileStorage", totally_unknown_kwarg=True)


def test_non_storage_backend_returns_error():
    """实例不是 StorageBackend 报错."""
    with pytest.raises(StageflowStorageError, match="StorageBackend"):
