# pavoz Storage Backend

🇨🇳 简体中文版 — 待补 (当前仅英文版)

pavoz 核心 **不带任何 storage driver** (`pip install pavoz` = 仅 stdlib)。
...
### 错误信息 (5 类, 均抛 `PavozStorageError`)

```
# 1. spec 格式错 (没有 : 也没有 .)
PavozStorageError: storage spec 'nocolon' 格式错误. 期望 'pkg.module:ClassName' 或 'pkg.module.ClassName'

# 2. module 未装
PavozStorageError: storage 'foo.bar:Baz' 模块 'foo.bar' 不可用: No module named 'foo'.
  检查 (1) 模块名拼写; (2) 对应 pip 包是否已安装 (e.g. foo).

# 3. class 名错
PavozStorageError: storage 'pavoz.storage:WrongClass' 类 'WrongClass' 不在模块 'pavoz.storage'.
  模块导出: ['FileStorage', 'StorageBackend', 'key_sha256']

# 4. kwargs 错
PavozStorageError: storage 'pavoz.storage.FileStorage' 实例化失败:
...
PavozStorageError: storage 'builtins:int' 实例不是 StorageBackend (类型 int)
```
...
# storage_kwargs = {dsn: "postgresql://...", table: "pavoz_kv"}

storage = load_storage("myapp.adapters:PostgresStore",
                       dsn="postgresql://...", table="pavoz_kv")
```

## 与之前 v0.4 contrib 的关系

v0.4.0 (commit `c7f760b`) 在 pavoz 内 vendor 5 个 adapter (Sqlite/Postgres/
MySQL/Redis/MinIO), 走 extras 机制 + contrib 子包。**v0.4.1 (2026-09-05) 反转** —
这是 over-engineering, 我们不该替客户决定需要哪些 DB/S3 client。
`pavoz.contrib.storage` 子包及 5 个 adapter 已删除, 改走 `load_storage`。
git history 保留两次方向作为 pavoz 设计演化的真实记录。

如果社区想用 contrib 模式, 走**独立包** (`pavoz-storage-postgres` 等),
自己维护, 不进 pavoz core。
