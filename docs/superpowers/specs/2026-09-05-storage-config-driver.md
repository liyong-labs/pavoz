# stageflow Storage Loader v0.4 Spec (反转: 取消 contrib, 改 config driver)

> v0.4 重新设计 (2026-09-05 user 拍板): 取消 `stageflow.contrib.storage` (已 ship 在
> commit `c7f760b`, 此 spec 描述反转方向). 改: core 加 `storage_loader.load_storage()` —
> 用户 config 写 `"pkg.module:Class"`, stageflow 运行时 importlib 加载.

## 背景 (反转原因)

v0.4 第一版 (`c7f760b`) 在 stageflow 内 vendor 5 个 adapter (Sqlite/Postgres/MySQL/Redis/Minio), 走
...
- pluggy 为 1400+ plugin 设计, stageflow 一个 storage slot 用它 = 牛刀杀鸡
- 推荐组合: config string + importlib + 友好 ImportError + 可选 entry_points 注册

## 设计

### 核心 API: `stageflow.storage_loader.load_storage()`

```python
# stageflow/storage_loader.py — stdlib only
...
__all__ += ["load_storage", "StageflowStorageError"]
```
...
`stageflow/contrib/storage/` 子包及 5 个 adapter + 7 个 tests + pyproject extras **全部删除**. 后续如果社区想用 contrib 模式, 走**独立包** (`stageflow-storage-postgres` 等), 自己维护, 不进 stageflow core.
...
- URL scheme dispatch (`postgresql://` 自动 pick driver) — stageflow 不是 db connection, 用普通 dotted path 即可
- 自动发现 / 自动转换 — 增加复杂度无收益

## 验收

- `pip install stageflow` — 仅 stdlib + core, 不带任何 driver
...
- 删 `stageflow/contrib/` 后, 38 core tests + 新增 storage_loader tests 全过
...
- **tag v0.4.0 还在 `c7f760b`** — 这意味着 `pip install stageflow==v0.4.0` 拉到的代码是"已废除的 contrib 设计"
- 反转需要**新 commit 链**:
  - 删 `stageflow/contrib/` 子包 (含 `_base.py`, `__init__.py`, `sqlite.py`, `postgres.py`, `mysql.py`, `redis.py`, `minio.py`)
  - 删 `tests/contrib/test_*.py` 和 `tests/contrib/__init__.py`
  - 删 `tests/test_base.py` (若已合并 — 实际在 `tests/contrib/test_base.py`)
  - 恢复 `pyproject.toml` (移除 7 个 extras; 移除 `[tool.setuptools]` 加的 `stageflow.contrib`; 移除 markers 注册)
  - 恢复 `stageflow/contrib/__init__.py` 删除 (或保留空目录?)
...
- 注意: 这是**反转**, 不是 "新增". git history 保留两次方向, 作为 stageflow 设计演化的真实记录.