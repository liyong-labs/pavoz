...
**Goal:** 删 stageflow v0.1.1 中零调用的死代码 (ponytail 审计识别的 YAGNI 项), 简化防御性包装, 不动 API 冻结面。
...
- 改前确认: 涉及项 `grep -r` 在 stageflow/ tests/ docs/ dags/ 全无调用点 (零调用 = 安全删)
...
```bash
cd stageflow && grep -rn "TaskTrigger\|TaskRef" stageflow/ tests/ docs/ dags/ --include="*.py" --include="*.md"
```
...
```bash
cd stageflow
rm stageflow/trigger.py
```

打开 `stageflow/__init__.py`。删 `from .trigger import TaskRef, TaskTrigger` 一行; `__all__` 列表删 `"TaskRef",` 和 `"TaskTrigger",`。
...
```bash
cd stageflow && grep -rn "ctx\.log\|ctx\.remaining_seconds\|ctx\.logger\." stageflow/ tests/ dags/ --include="*.py"
```
Expected: 0 hit

- [ ] **Step 2: 删 Ctx.log 和 Ctx.remaining_seconds 方法**

打开 `stageflow/runtime.py`。`Ctx` 类内, 删以下两块:
...
```bash
cd stageflow && grep -rn "\.metadata\|stage_metadata\|stage.*metadata" stageflow/ tests/ dags/ --include="*.py"
```
Expected: 0 hit (除定义)
...
```bash
cd stageflow && grep -rn "\.entrypoints\|\.depth_of" stageflow/ tests/ dags/ --include="*.py"
```
Expected: 0 hit
...
打开 `docs/quickstart.md`。"## CLI 命令 (v0.1)" 表删 inspect 行 (`python -m stageflow inspect ...` 一行 + 描述)。
...
```bash
cd stageflow && python -m stageflow --help 2>&1 | head -5
```
Expected: usage 文本 (argparse "stageflow" + subcommands 列表)

- [ ] **Step 2: 删 __main__.py + 改 __init__.py**

```bash
cd stageflow && rm stageflow/__main__.py
```

打开 `stageflow/__init__.py`, 末尾追加:
...
- [ ] **Step 3: 验证 python -m stageflow 仍工作**

```bash
cd stageflow && python -m stageflow --help 2>&1 | head -5
cd stageflow && python -m stageflow run --help 2>&1 | head -3
```
...
```bash
cd stageflow && python -m ruff check stageflow/ tests/ docs/
```
...
```bash
cd stageflow
git add stageflow/ docs/quickstart.md CHANGELOG.md
...
```bash
cd stageflow && git tag -l && git log --oneline -3
wc -l stageflow/*.py tests/*.py | tail
```
